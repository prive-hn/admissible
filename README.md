...
| `tests/` | 2934 kernel/server/context/project/class/RGA/calibration/bench/schema/paper/custody tests, including 2445 for the developer product |
...
3042 checks green in this repository, in two scopes that are counted
separately because they prove different things:

| Scope | Checks | Where |
| ----- | -----: | ----- |
| Research kernel — the number the paper cites | 597 | 489 in `tests/` + 37 `atlas/tests/` + 71 `apps/cockpit/tests/` |
| Developer product (`admissible/`) | 2445 | `tests/test_admissible_*.py` + the split suites under `tests/architecture/`, `tests/core/`, `tests/ready/`, `tests/trust/` and `tests/compatibility/` |
| **Total** | **3042** | 2934 under `tests/`, 37 atlas, 71 cockpit |
