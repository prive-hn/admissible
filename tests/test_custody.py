...
class PolarityD6(unittest.TestCase):
    """D6/T3: seal raises, escape lowers, and — since the CF1 repair — the
    only event that raises a line's standing back is an attributed
    `cal_resolve`. A discredit is standing-neutral and a refusal lowers only."""

    def test_seal_raises_escape_lowers_and_only_a_resolution_raises(self):
        ...
        h.cal.replay_run(second.index, "refuted", "other")      # cal_discredit: '0'
        self.assertFalse(h.cal.admissible("w"))                 # the demonstrated escape stands
        ...
        self.assertEqual(custody.polarity_of("cal_discredit"), "0")   # reaches no established escape
        self.assertEqual(custody.polarity_of("cal_resolve"), "+")     # the one attributed raise
...
class AnchorsReadAsReplayReads(unittest.TestCase):
    ...
    def test_a_refusal_group_is_exposed_and_its_deletion_only_un_taints(self):
        """A same-class stamp anchors the escape before it. The refusal group
        is anchored by nothing at all: no standing reader consults the refusal
        registry any more, so deleting the group rebuilds every stamp
        unchanged and its whole effect — the taint — disappears. That is the
        unauthenticated-history residue one event class wider than before the
        CF1 repair, and closing it takes the authenticated publication path
        rather than replay."""
        h = CalHarness(); h.declare_tests(); h.seal_line("w")
        h.tier_a_escape("w", nonce="e1")
        h.seal_line("x")                                         # same class: its stamp anchors the escape
        ...
        surface = custody.deletion_surface(h.cal, "w")
        escape = [e for e in surface if e.reason == "escape"]
        self.assertTrue(escape and all(not e.exposed for e in escape))      # anchored by x's stamp
        taint = [e for e in surface if e.reason == "taint"]
        self.assertTrue(taint and all(e.exposed and not e.anchored_by for e in taint))
        ...
        self.assertTrue(cal2.impeached("w"))                     # the escape never depended on the refusal
    ...
    def test_a_refusal_reaches_no_escape_and_the_ratchet_still_owes_it(self):
        """The CF1 repair removes an accidental anchor, and the cost is stated
        rather than hidden. A refusal used to void the refused checker's
        established escape, which emptied a later install's coverage
        obligation — so the install recomputed against the refusal and
        anchored it. Now the escape survives its checker's refusal, so the
        obligation stands: an install whose ledger model omits the escape's
        derived id is refused live, and the refusal group anchors nothing."""
        from rga.core import AdmissionPolicy, ClassAdmission
        h = CalHarness(); h.declare_tests(); h.seal_line("w")
        run = h.tier_a_escape("w")
        h.fcd_open("z"); h.a.open("z", "gen", "temp=0.7")
        h.fcd_write("z"); h.sample("z", b"z0"); h.trial("z")
        h.a.replay("z", 0, "refuted", "w-same")              # refuses `tests`: taints w only
        self.assertTrue(h.a.tainted("w"))
        self.assertTrue(h.cal.impeached("w"))                # the escape is untouched
        h.a.declare(Refuter("tests", "v2", "tester", "ledger"))
        ...
        succ = AdmissionPolicy({"impl": ClassAdmission(
            ...
            k=K, theta=1.0, p_min=0.5, excluded=frozenset({"refuter_source", "refuter_results"}),
            residual=(("correct fix", "check_stage"),))}, version="r2")
        with self.assertRaises(ValueError):                  # forgetting is loud again (C4)
            h.cal.install(succ)
        taint = [e for e in custody.deletion_surface(h.cal, "w") if e.reason == "taint"]
        self.assertTrue(taint)
        self.assertTrue(all(e.exposed and not e.anchored_by for e in taint))
        drop = {e.index for e in taint}
        adm2 = Admission.from_events([e for i, e in enumerate(h.a.events) if i not in drop],
                                     h.e, admission_policy())
        self.assertFalse(adm2.tainted("w"))                  # deletion un-taints: the residue, widened
        cal2 = CalibrationAuthority.from_events(list(h.cal.events), adm2, h.cal.policy)
        self.assertTrue(cal2.impeached("w"))                 # and the escape still stands
    ...
    def test_a_refusal_props_up_no_other_line(self):
        """The property that made a refusal group non-exposed is gone with the
        bug that created it. A refusal used to void a tier-B escape by the
        refused checker against a DIFFERENT sealed line v (on which the
        checker was not pinned), leaving v admissible and so making the
        refusal load-bearing for v: deleting it cost v. Validity no longer
        reads the refusal registry, so v stays impeached throughout, the
        refusal props up nothing, and its group is freely deletable by
        whoever holds the record — the residue, stated."""
        from rga.core import AdmissionPolicy, ClassAdmission
        h = CalHarness(); h.declare_tests()
        h.a.declare(Refuter("hawk", "v1", "hawk-author", "ledger"))
        ...
        self.assertFalse(h.cal.admissible("v"))
        h.a.replay("w", 0, "refuted", "diverge")               # refuse tests: taints w, and only that
        self.assertFalse(h.cal.admissible("w"))
        self.assertFalse(h.cal.admissible("v"))                # v was never propped up
        taint = [s for s in custody.deletion_surface(h.cal, "w") if s.reason == "taint"]
        self.assertTrue(taint)
        self.assertTrue(all(s.exposed and not s.anchored_by for s in taint))
        ...
        self.assertTrue(alone.admissible("w"))                 # w raised by the deletion
        self.assertFalse(alone.admissible("v"))                # at no cost to v
...
class SupportIncludesValidityDegraders(unittest.TestCase):
    ...
    Since the CF1 repair there is exactly one such event class — the
    attributed `cal_resolve` that voids a contested escape — because a
    discredit and a refusal no longer reach what a checker demonstrated."""

    def test_deleting_a_void_resolution_revives_the_escape(self):
        h = CalHarness(); h.declare_tests(); h.seal_line("w")
        ...
        self.assertIn("cal_resolve", {t for _, _, t in sup.positive})
        drop = {i for j, i, t in sup.positive if j == "cal" and t == "cal_resolve"}
        ...
        self.assertFalse(rebuilt.admissible("w"))                  # the escape stands again


class RepairedCF1DiscreditIsStandingNeutral(unittest.TestCase):
    """Finding CF1, closed. One party holding the sealed bytes (B12) used to
    file a second tier-A escape with any witness, replay it divergently, and
    have `_check_valid` void every run of that checker — including the first
    escape, established by identical replay. The line was un-impeached, the
    refuter un-demoted, nothing tainted and `Admission.refused` empty: a
    single-party report raising standing, against C3. The transition table
    (paper/RGA/INVARIANTS.md §8.2, ReplayEscape) always said only
    UNESTABLISHED escapes of a discredited checker were void; the code now
    agrees with it, and a discredit is standing-neutral."""

    def test_a_self_discredit_leaves_the_demonstrated_escape_standing(self):
        ...
        self.assertTrue(first.established)                             # its identical replay stands
        self.assertTrue(h.cal.impeached("w"))
        self.assertFalse(h.cal.admissible("w"))
        self.assertTrue(h.cal.demoted("tests", "v1", "impl"))
        self.assertEqual(custody.polarity_of("cal_discredit"), "0")

    def test_the_only_raise_is_a_contest_resolved_by_a_named_actor(self):
        h = CalHarness(e_max=0); h.declare_tests(); h.seal_line()
        first = h.tier_a_escape(nonce="e1")
        h.cal.replay_run(first.index, "survived", "not-a-kill")        # contests THIS run
        self.assertTrue(h.cal.contested(first.index))
        self.assertTrue(h.cal.impeached("w"))                          # fail-closed while contested
        h.cal.resolve(first.index, "owner", "void", "flaked on these bytes")
        self.assertTrue(h.cal.admissible("w"))
        self.assertEqual(custody.polarity_of("cal_resolve"), "+")


class RepairedCF14RefusedCheckerRefusedOnRebuild(unittest.TestCase):
    """CF14, closed. `from_events`' cal_run branch omitted the
    `Admission.refused` check the live guard makes, so a filing by an
    already-refused checker was refused live and accepted on rebuild. The
    filing now records the scrutiny cut it was written at and rebuild
    evaluates the checker's standing there, so both paths agree."""

    def test_filing_by_a_refused_checker_is_refused_on_both_paths(self):
        ...
        forged = list(h.cal.events) + [{"type": "cal_run", "run_index": len(h.cal.runs), "line_id": "w",
                                        ...
                                        "verdict": "refuted", "witness_hash": "k", "finder": "f",
...
class RepairedCF1RefusalPathLowersOnly(unittest.TestCase):
    """CF1's second path (T5), closed. A divergent `Admission.replay` refuses a
    checker everywhere, and `_check_valid` used to void its escapes at every
    position — so a tier-B checker's accepted escape against w was voided by
    its refusal on another line, un-impeaching w with no taint (w never
    pinned it). Validity no longer reads the refusal registry, so the refusal
    lowers standing or leaves it alone, never raises it."""

    def test_refusal_of_a_tier_b_checker_leaves_its_escape_standing(self):
        from rga.core import AdmissionPolicy, ClassAdmission
        h = CalHarness(e_max=0); h.declare_tests(); h.seal_line()
        h.a.declare(Refuter("hawk", "v1", "hawk-author", "ledger"))
        ...
        self.assertTrue(h.cal.impeached("w"))                    # the accepted escape stands
        self.assertFalse(h.a.tainted("w"))                       # w never pinned hawk
        self.assertFalse(h.cal.admissible("w"))


class RepairedCF2ReplaySeam(unittest.TestCase):
    """Finding CF2, closed. `CalibrationAuthority.from_events` re-checked
    `_guard_audit_checker` against `escapes(cls)` at the FINAL registry, so an
    honest journal in which an audit was filed by a checker refused LATER in
    Admission was refused on rebuild — the live machine accepted what replay
    refused. Validity no longer degrades on refusal at all, so the escape that
    qualified the audit's checker still qualifies it on rebuild."""
    ...
    def test_honest_journal_with_an_audit_by_a_later_refused_checker_replays(self):
        from rga.core import derive_seed
        h = CalHarness(claims=self.CLAIMS); h.declare_tests()
        h.a.declare(Refuter("hawk", "v1", "hawk-author", "ledger"))
        h.a.measure("hawk", "v1", DefectModel(D1, "mutator"), ledger(8, 10))
        ...
        rebuilt = rebuild(h, claims=self.CLAIMS)
...
class RepairedCF6SortedFloor(unittest.TestCase):
    ...
    claim whose kernel-counted power was 0/|D|: the floor compared a
    projection that had forgotten its sort. The floor now reads the weakest
    sort present, so a declaration cannot clear what a measurement failed,
    and the seal carries each sort's coordinate with the floor's realizer
    named."""

    def test_a_declared_figure_no_longer_clears_a_floor_the_ledger_failed(self):
        BND = ("bnd", "v1")
