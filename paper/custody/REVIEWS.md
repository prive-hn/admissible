...
The repository's pull-request reviewer read the companion after the branch was marked ready and, in sixteen passes (each on the repaired head, on both the public and the internal pull request), returned thirty findings — twenty-eight against `custody.py`, one against the paper index, and one against the paper's replay-language definition — all real, all applied. Each code finding carries a regression test that fails on the previous companion, except two whose exact trigger is contrived under the one-class harness (R4-20, R4-22): those are verified by the re-derivation net and the closure fuzz and documented. The count guards moved with the added checks.
...
| R4-18 | P2 | `_install_anchors_for_escape` removed installs from its probe by `policy_version`, so two installs of the same version (the live path allows a re-install when content is unchanged) were treated as one, and had a later same-version install covered a run an earlier one did not, the probe would over-state the deletion. | applied — the probe removes installs by journal index, so same-version installs are distinguished (`deletion_closure`'s minimisation was already index-based, so its reported set was minimal on the previous head too). The over-inclusion is in fact unreachable: a defect model's ledger is written once per (refuter, model) (`rga/core.py:_guard_power_once`, V13), so a re-install cannot cover a corpus entry created after the first measure, and two same-version installs over one corpus are both necessary or both harmless — the index-based probe is applied for correctness regardless, as R4-7's documented case. |
| R4-19 | P2 | A `cal_run` filed before another line's `cal_stamp` but established only after it: deleting the run shifts the stamp by one, but `_alt_delete` rewrote only `run_index`, leaving `track_records[*].as_of` (which records the stamp's own `_position()` and `from_events` recomputes) at the old value, so the recompute refused an otherwise valid renumbering and the run was wrongly reported non-exposed with a non-replayable closure. | applied — `_alt_delete` resets each shifted stamp's `track_records[*].as_of` to its new position, completing the consistent renumbering the move set allows (T12c); `test_a_run_filed_before_a_later_stamp_is_exposed_once_the_stamp_shifts` shows the run exposed, the deletion rebuilding, and the un-shifted alternative refused; a fuzz that files escapes before later seals and replays them afterward covers the shape |
| R4-20 | P2 | `_install_anchors_for_escape` removed all installs to find the jointly necessary set, but a downstream `cal_close(E5)` reads the budget an install set, so removing the installs without their E5 closes replayed the close under the pre-install budget and refused; the probe then found no anchors and `deletion_closure` reported a non-rebuilding group (an unrelated budget install predating the escape triggers the same refusal). | applied — the probe now carries each removed install's downstream E5 closes (`_install_e5_candidates`) while testing whether removing installs repairs the alternative, so the base rebuild succeeds and the add-back isolates the truly necessary installs; `deletion_closure` then minimises which E5 the deletion needs. The exact trigger (a charge count strictly between two budgets under a one-class policy) is contrived, R4-13's regime, so the fix is covered by the closure-completeness fuzz over budget-and-coverage installs with E5 closes rather than an end-to-end test |
| R4-21 | P2 | A refusal group followed by a mediated seal under a switched pin: deleting the RGA group shortens the admission journal and shifts that later seal's position, but the surviving `cal_stamp`'s `sealed_at` (and any close/exclude/install `as_of`) kept its original admission position, so `_rebuild_alt` refused the advertised closure as a stamp not bound to its seal. | applied — `_shift_adm_positions` rewrites each surviving cal event's admission-position field by the RGA deletions before it, and `_rebuild_alt` applies it after deleting RGA events, completing the cross-journal renumbering (T12c). `test_a_taint_deletion_shifts_a_surviving_stamps_admission_position` builds the switched-pin seal after a refusal and shows the closure rebuilds only with the shift; it fails on the previous companion |
| R4-22 | P2 | When a previously installed admission policy is reinstalled as the current one, `adm._policies.values()` keeps first-insertion order, so its last element can be a different policy; `_rebuild_alt` supplied that as the last policy to `Admission.from_events`, which takes the last supplied policy as live, leaving the wrong policy current and failing calibration replay's coverage check. | applied — `_rebuild_alt` orders the policies so the live `adm.policy` is supplied last (repeating its version when it was reinstalled). The misordering is real (an extra-class install then a reinstall of the initial leaves `values()` last ≠ live) but its end-to-end trigger needs a multi-class admission reinstall plus a taint deletion in the closure, contrived under the one-class harness, R4-18's regime, so it is documented and the reorder is exercised by the cross-journal fuzz |
| R4-23 | P2 | `frechet_bounds([], 'intersection')` returned `(0.0, 0.0)`, though the empty intersection is vacuously satisfied (probability 1) and the module's own `power_joint([])` returns 1.0; a caller folding a dynamically empty conjunctive set read a contradictory zero-power interval. | applied — emptiness is resolved per event: an empty union is `(0.0, 0.0)`, an empty intersection `(1.0, 1.0)`. `test_the_empty_conjunction_is_the_identity_of_its_event` pins both and that they agree with `power_joint`; it fails on the previous companion |
| R4-24 | P2 | `deletion_closure` initialised from `group_with` and the anchors but ignored `redundant_with`, so for one of several establishing replays of a run it returned an empty tuple; deleting the replay and that closure left a sibling establishing the run and the line still impeached. | applied — for a replay with siblings the closure carries every sibling (never pruned) and, the run then no longer an established escape, the readers of that collective removal (a later stamp, E5 or audit) computed as for the run's own deletion, then minimised by re-derivation. `test_a_redundant_replays_closure_names_its_siblings_and_their_reader` shows the closure naming the siblings and a same-class stamp and the deletion raising standing; it fails on the previous companion |
| R4-25 | P2 | The R4-24 redundant-replay closure carried a run's sibling replays but, for a tier-B run, not its accepting `cal_adjudicate`: removing every establishing replay left the adjudication behind, which `from_events` rejects ('adjudication requires an established escape'), and an exclusion naming the now-unestablished run was not rewritten because `_invalidates` still saw multiple replays in the original journal. | applied — `_rebuild_alt` computes the invalidated runs from the whole cal deletion set (`_runs_unestablished_by`: every establishing replay deleted, or the tier-B adjudication), so an exclusion naming a collectively-unestablished run is rewritten (T12c/D16); and the redundant-replay branch of `deletion_closure` carries the run's adjudication with its siblings. `test_a_tier_b_runs_redundant_replays_carry_its_adjudication` builds a tier-B escape replayed twice then adjudicated: the closure names the sibling and the adjudication, the deletion rebuilds and raises standing, and the sibling-only alternative is refused; a tier-B redundant-replay fuzz with exclusions and stamps reports every closure coherent. It fails on the previous companion |
| R4-26 | P2 | The R4-24/25 redundant-replay and R4-9 refusal-group closures added an anchored `cal_install`'s later E5 closes but not a *further* install whose coverage the anchor's removal breaks: removing an install anchor renumbers a later run, shifting its position-derived defect id, so a subsequent install that covered it by the old id fails `_guard_install_covers` on rebuild; `deletion_closure` omitted that downstream install and the advertised closure did not rebuild. | applied — after the analytic minimisation, `deletion_closure` runs a completeness net: if the alternative still does not rebuild it finds the jointly necessary further installs by re-derivation (remove all installs not yet in the closure, add back each the rebuild does not need) and carries them with their own E5 readers, then re-minimises; if no install helps the group stays incoherent (kept off the exposed set). The exact trigger — a refusal-revived escape whose install anchor's deletion renumbers a covered run out of a subsequent install's ledger under a one-class policy — is contrived (the R4-13 regime), so the net is covered by the closure-completeness fuzz over refusals, installs, budget changes and coverage rather than an end-to-end test |
| R4-27 | P2 | D5 claimed the kernel's three `from_events` "recognize exactly `L_rep`", but the report's own §8a shows they do not: F14 has `from_events` accept a `cal_run` filed after its checker's refusal — outside `L(μ)`, so outside `L_rep` (unsound) — and F2 has it reject a divergent-replay audit journal that is in `L_rep` (incomplete). The later identification of the computed surface with `L_rep` rested on that over-claim. | applied — D5 now states that `from_events` decides `emit(r) = r`, recognising the *re-derivable* records, a decidable approximation of `L_rep` that differs from it at exactly the F2 and F14 seams; a "what D5 does not say" notes the exact recognizer would re-impose F14's guard and admit F2's seam (catalogued §8a/N28, not assumed), and the results reading the surface as `L_rep` are qualified as holding relative to `from_events`, the repository's replay operator |
| R4-28 | P2 | `bonferroni_horizon` rounded `1/(1-p)` to nine places before `ceil`, snapping a genuine near-boundary excess to the boundary: `bonferroni_horizon(0.900000000001)` returned 10 though `power_joint([0.900000000001]*10)` is 1e-11 > 0, so the assumption-free bound has not yet reached its fail-closed zero and the horizon is 11. | applied — the horizon is now defined against `power_joint`'s own rounding (the deficit `1 - n(1-p)` rounded to 12 places): it walks from `floor(1/(1-p))` to the first `n` that zeroes and back, so representation noise around an exact integer still collapses (p = 0.9 → 10) but a real deviation does not. `test_the_horizon_is_where_power_joint_first_zeroes` pins the near-boundary case and that the horizon is exactly where `power_joint` first reaches zero across a range of p; it fails on the previous companion |
| R4-29 | P2 | `power_joint`'s floating-point correction rounded the residual `1 − Σ(1−p)` to 12 places, which rounds a positive residual *upward* and over-claims the assumption-free *lower* bound: `power_joint([0.9999999999996])` returned 1.0 (a one-element conjunction is its own power) and `power_joint([0.5000000000003]*2)` returned 1e-12 though the Fréchet bound is ≈6e-13 — the R4-28 horizon fix inherited the same rounding. | applied — the correction is now a *downward* clamp only: a residual within the sum's O(n·ε) accumulation error of zero reads the fail-closed horizon zero (ten 0.9s still 0), every other residual is returned exactly and never lifted to the 12-place grid, and `bonferroni_horizon` is decided by `power_joint` itself so the two cannot disagree. `test_the_joint_reading_never_rounds_a_positive_bound_up` pins the two over-claim cases and checks, across a 20k random sweep against exact rational arithmetic, that the reading never exceeds the true bound; it fails on the previous companion |
| R4-30 | P2 | The R4-26 completeness net added a jointly-necessary install's downstream `cal_close(E5)` readers straight into the closure, after the candidate-pruning loop had already run, so an E5 close still valid under the preceding budget was reported in `deletion_closure` and overstated the exact deletion cost. | applied — the net now adds the installs and their E5 readers to the *candidate pool* rather than the closure, and a single minimisation runs after it, to a fixpoint, so every candidate — anchors, installs and the net's E5 readers alike — is dropped when the alternative rebuilds without it. Same reachability regime as R4-26 (contrived under a one-class policy), so covered by the closure-minimality fuzz over refusals, installs, budget changes and coverage rather than an end-to-end test |
...
## Round 7 — internal adversarial re-review (six-reviewer workflow)

When the pull-request reviewer (Codex) reached its usage limit after the automated review's sixteenth pass, the same job — find real, reproducible defects — was run again by six independent adversarial reviewers, one per dimension of the companion (the deletion surface and anchor enumeration; closure completeness and minimality; the power algebra; groups, rewrites and invalidation; the standing/verifier/exposure queries; and the taint / cross-journal branch), each required to build an executable repro through the kernel's live transitions, followed by an independent skeptic that reproduced and confirmed or rejected each candidate. Two findings survived verification (one raised, and independently corroborated, by two reviewers), both real and reachable, both applied:

| id | severity | finding | disposition |
|---|---|---|---|
| R7-1 | P2 | `exposed()` / `deletion_surface` reported a refusal (taint) group as "deletable at no cost to any other line" when its deletion lowers a *different* line's standing. A refusal is the F1 second path (T13(b), `POLARITY['rga_refuse']="±"`): it voids every escape by the refused checker, so when that checker had a valid tier-B escape against another sealed line it does not taint, that line is currently un-impeached and admissible, and deleting the refusal revives the escape and impeaches it — with no rebuild refusal, so the stamp/install readers and the (unconditional-for-taint) `coherent` net all miss it. The companion's own `support`/`_degraders` already list that refusal as a positive atom of the other line, so the surface was internally inconsistent with the support code. | applied — the refusal-group branch of `_anchors_of` now also anchors the group by the `cal_run` of any refuted run by the refused checker that is valid but for the refusal, on another currently-admissible sealed line the refusal does not taint; the group is then non-exposed, and `deletion_closure` carries that escape's witnesses so raising the tainted line costs the other line nothing. `test_a_refusal_propping_up_another_admissible_line_is_not_exposed` builds the scenario through kernel transitions (a pin switched to a second checker, a cross-line tier-B escape, then the refusal) and shows `exposed` empty, the group anchored, and the group-alone deletion lowering the other line; a 500-journal cross-line soundness fuzz reports zero exposed groups whose deletion lowers another line (406 violations on the previous companion). It fails on the previous companion |
| R7-2 | P2 | `deletion_closure` over-included the sibling replay for a tier-B redundant-replay escape. Once the target replay is deleted, deleting the accepting `cal_adjudicate` alone invalidates the run (an unadjudicated tier-B run is not a valid escape), so the sibling is not needed; but R4-25 put both the siblings and the adjudication in the un-prunable `struct` set, and the minimisation loop iterates only `cand`, so the redundant sibling was never dropped — overstating the deletion cost against the function's own minimality contract. | applied — for a tier-B redundant replay the adjudication goes in `struct` (it is required, and being deleted is not orphaned) while the sibling replays go into the prunable `cand`, so the re-derivation minimisation drops them; tier A is unchanged (no adjudication shortcut, every replay is required). `test_a_tier_b_runs_redundant_replays_carry_its_adjudication` now asserts the minimal closure names the adjudication alone; a tier-B redundant-replay minimality fuzz reports every closure complete and minimal. It fails (as an over-inclusion) on the previous companion |

## Round 8 — second internal adversarial re-review of the Round-7 head (six-reviewer workflow)

The Round-7 fixes were themselves put back through the same six-reviewer job — one adversarial reviewer per dimension of the companion, each building an executable repro through the kernel's live transitions, each finding then reproduced or rejected by an independent skeptic. Two findings survived verification (each independently raised and corroborated by two of the six reviewers), both real and reachable, both applied; the four soundness, re-derivation and remaining dimensions returned clean.

| id | severity | finding | disposition |
|---|---|---|---|
| R8-1 | P1 | The Round-7 R7-2 fix (tier-B: adjudication required, establishing replays prunable) reached only the `redundant_with` surface-event branch of `deletion_closure`, not the *anchor* branch that the R7-1 fix routes an F1-second-path run through. `_anchors_of` anchors a refusal on the revived tier-B escape's `cal_run`; the anchor loop then ran that run through `_run_structural`, dumping its `cal_run` + establishing replay + accepting adjudication into the un-prunable `struct` set, while the minimisation iterates only `cand`. So the establishing replay could never be pruned, and `deletion_closure` returned a non-minimal set (the R7-1 closure carried `cal_replay` though deleting the accepting `cal_adjudicate` alone invalidates the run), overstating the deletion cost T11's certificate reasoning rests on. | applied — the anchor branch now applies the R7-2 rule: for a tier-B run reached as an anchor the accepting adjudication goes in `struct` (required) and the run's `cal_run` and establishing replays go into the prunable `cand`, so the re-derivation minimisation drops them; tier A (and an unadjudicated tier-B run) keep every structural event. `test_a_refusal_props_closure_carries_the_adjudication_not_the_replay` rebuilds the R7-1 scenario and asserts the closure names the adjudication and not the replay, and still rebuilds raising the tainted line at no cost to the other. It fails (as an over-inclusion) on the previous companion |
| R8-2 | P1 | `bonferroni_horizon(p)` seeds `n = floor(1/(1−p))` and evaluates `power_joint([p] * n)`, materialising a length-`n` Python list. A bounded refuter's carried figure `1 − (1 − ε)^N` (D13) can sit legitimately close to 1 — e.g. `ε=0.2, N=115` gives `p ≈ 0.99999999999`, a valid seal composite / `power_min` — for which `n ≈ 1.4·10¹¹`, so the list allocation (~1 TB) raises `MemoryError` before any horizon is returned. The horizon is a finite integer obtainable in closed form; a legal query to the documented API crashed instead of returning it. | applied — `bonferroni_horizon` now uses `_power_joint_uniform(p, n)`, which computes `power_joint([p]*n)` as `1 − n·(1−p)` under the identical downward clamp (the sum's O(n·ε) accumulation error is exactly the clamp scale, so the closed form and the list build agree on the sign at the horizon), in O(1) space. `test_the_horizon_of_a_near_one_power_is_finite_and_needs_no_giant_list` returns a finite horizon for `1 − 0.8¹¹⁵`, checks the defining zero/positive boundary there, and verifies closed-form/list agreement across a 2 000-case sweep and the pinned small-p horizons. It raises `MemoryError` on the previous companion |

## Round 9 — third internal adversarial re-review of the Round-8 head (six-reviewer workflow)

The Round-8 head went through the same six-reviewer job once more, with each finding given a P0/P1/P2 priority. One blocking defect surfaced (independently raised by two reviewers) — the Round-8 R8-2 fix was incomplete — and one non-blocking P2; the soundness, completeness, minimality and re-derivation dimensions returned clean, and the reviewers re-verified that `_power_joint_uniform`'s sign never disagrees with `power_joint` at any reachable p.

| id | severity | finding | disposition |
|---|---|---|---|
| R9-1 | P0 | The R8-2 fix removed `bonferroni_horizon`'s `~1/(1−p)`-length list (the `MemoryError`) but left the horizon decided by a *linear scan*: it still seeded `n = floor(1/(1−p))` and back-walked one `n` at a time. For `p` near 1 the clamp `4·n·ε` makes `_power_joint_uniform` read zero over a contiguous band of width `≈ 4·ε/(1−p)²` below the seed, so the down-scan runs that many iterations — `~10¹²` at `p = 1 − 0.5⁴⁵` (a legal bounded `ε=0.5, N=45` figure), effective non-termination. The space crash was traded for a time hang on the same near-1 input class, one bounded-`N` step past what the R8-2 test exercised (`1 − 0.8¹¹⁵`, which finished in `~10 s` and masked it — the whole custody suite ran in `~9 s`). | applied — `bonferroni_horizon` now seeds at the *clamp-shifted* boundary `n = floor(1/((1−p) + 4·ε))`, the closed-form point where `_power_joint_uniform` turns zero, so the ± refinement is O(1) (3 evaluations for every input tried, including `nextafter(1,0)`); the horizon value is unchanged where the old scan could complete (0 mismatches, p ≤ 0.99). `test_the_horizon_of_a_near_one_power_is_finite_in_bounded_work` caps `_power_joint_uniform` evaluations and asserts convergence in < 64 for a range of near-1 figures up to the largest double below 1; it fails fast (the cap raises) on the previous companion, and the suite now runs in ~0.5 s |
| R9-2 | P2 | `support().positive` (via `_degraders`) over-includes a checker's `cal_discredit` / diverged `cal_replay` as a positive atom of a line for a refuted run that was never a *valid* witness — e.g. an unadjudicated tier-B escape, which never impeaches. Deleting those atoms revives nothing and lowers no standing, so they are not degraders (T17(i)/D26 name the degraders of *voided* witnesses). | open — non-blocking, not applied under the P0/P1 gate. The error is strictly *over-inclusive* (a genuine degrader is never missed), so `support` stays a superset and its determination property (events outside the support do not change the value) is preserved; and `_degraders` feeds only `support().positive`, never `exposed` / `deletion_surface` / `standing_certificate` / `verify_certificate`, so no soundness or gated output is affected. Informational inaccuracy in a non-gated research query, safe direction |

## Disposition

`applied` — taken into the next revision of DRAFT.md / IMPROVEMENTS.md / custody.py / tests. `open` — either a branch document's own theorem error (those documents are not in the repository; where the kernel fact behind it was carried into DRAFT.md the row says so), a kernel code change (catalogued as N1–N28, not applied), or a non-blocking companion finding held under the P0/P1 gate (R9-2). Round-1 minors are not tabulated.

| R7-1 | P2 | applied — companion and test |
| R7-2 | P2 | applied — companion (minimal closure) and test |
| R8-1 | P1 | applied — companion (minimal closure on the anchor path) and test |
| R8-2 | P1 | applied — companion (`bonferroni_horizon` off the list build) and test |
| R9-1 | P0 | applied — companion (`bonferroni_horizon` O(1) closed-form seed) and test |
| R9-2 | P2 | open — non-blocking; `support().positive` over-inclusive only, no soundness or gated output affected |
...
| R4-18 | P2 | applied — companion (index-based probe); unreachable, documented |
| R4-19 | P2 | applied — companion and test |
| R4-20 | P2 | applied — companion (E5 readers in probe); contrived trigger, fuzz + documented |
| R4-21 | P2 | applied — companion and test |
| R4-22 | P2 | applied — companion (live policy last); contrived trigger, documented |
| R4-23 | P2 | applied — companion and test |
| R4-24 | P2 | applied — companion and test |
| R4-25 | P2 | applied — companion and test |
| R4-26 | P2 | applied — companion (completeness net); contrived trigger, fuzz + documented |
| R4-27 | P2 | applied — paper (D5 qualified) |
| R4-28 | P2 | applied — companion and test |
| R4-29 | P2 | applied — companion and test |
| R4-30 | P2 | applied — companion (fixpoint minimisation); contrived trigger, fuzz + documented |
