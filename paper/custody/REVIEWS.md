...
The repository's pull-request reviewer read the companion after the branch was marked ready and, in ten passes (each on the repaired head, on both the public and the internal pull request), returned nineteen findings — eighteen against `custody.py` and one against the paper index — all real, all applied, the code findings each with a regression test that fails on the previous companion; the count guards moved with the added checks.
