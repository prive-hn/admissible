...
The repository's pull-request reviewer read the companion after the branch was marked ready and, in fifteen passes (each on the repaired head, on both the public and the internal pull request), returned twenty-nine findings — twenty-seven against `custody.py`, one against the paper index, and one against the paper's replay-language definition — all real, all applied. Each code finding carries a regression test that fails on the previous companion, except two whose exact trigger is contrived under the one-class harness (R4-20, R4-22): those are verified by the re-derivation net and the closure fuzz and documented. The count guards moved with the added checks.
