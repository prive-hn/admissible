...
| Seal(id) | `pc=Open`; `|samples| = k`; ∀ claim `j`, ∀ `r ∈ j.refuters`, ∀ `i<k`: trial `(r,j,i)` exists with `verdict=survived`; ∀ `r` used: some trial of `r` on this line has `replays ≥ 1`; ∀ ledger `r` of `j`: `P[(r, D_j)]` exists; ∀ bounded `r`: `P[(r,⊥)]` exists; ∀ `j`: `D_j`'s fixed author `≠ generator`; `id ∈ S` (FCD); a `residual` disposition of `check_stage` requires a Passed FCD check stage | compute `agreeing_j`, the per-sort coordinates and `composite_j`, and `floor_basis_j` as the minimum over the sorts present; if some `agreeing_j/k < θ` → Close(V2), naming in `miss_observed` exactly the refuters whose own witnesses differ from their sample-0 witness; elif `min_j floor_basis_j < p_min` → Close(V5); else `S_R[id] ← Seal`; `pc=Sealed` |
...
Composite power for claim `j`: let `L` be the ledger refuters of `j` with records against `D_j`, `B` the bounded refuters. If `|L| ≥ 2` their killed-id sets are unioned over the shared `D_j`: `u = |∪ killed|/size`. The seal carries the two sorts separately — `ledger_composite_j = u` when `L ≠ ∅`, `bounded_composite_j = max_{b∈B} power_b` when `B ≠ ∅` — and never collapses one into the other. `composite_j = max` of the sorts present, labelled `union` if only `L` contributed with `|L| ≥ 2`, `single` if one refuter, `max` otherwise: the strongest scrutiny applied. No product, no noisy-OR.

The **floor basis** `floor_basis_j` is the *minimum* over the sorts present, with `floor_witness_j` naming the contributor that realized it, and it is what the V5 gate compares. A ledger figure is counted by the kernel over a named finite defect model; a bounded figure is computed from a declared `(ε, N)`. The two speak about different alternatives, so a `max` across them bounds nothing jointly, and taking it let a declared `1.0` clear any floor over a `0/|D|` ledger — a declaration outranking a measurement. Where both sorts attack a claim each must therefore clear the floor on its own base. A claim attacked only by declarations still clears its floor by declaration, because the kernel cannot measure what nobody measured, and the witness records that so a reader can discount it.
...
- **R2** `id ∈ S_R ⇒` every power figure in `S_R[id]` equals a record `P[(r,D)]` written before Seal by Measure or Bound from a ledger or a declared `(ε,N)` the seal carries; no transition writes an existing key of `P`; hence no sealed power is raised after the fact. The seal keeps the kernel-counted and the declared figures in separate fields, and the floor it was gated on is the weaker of the sorts present, so no declared figure clears a floor on a claim whose measured figure does not.
...
- Per claim: `claim_id`, `spec_hash`, each refuter as `(id, version, mode, power, D_hash | ⊥, kills | ⊥, size | ⊥, ε | ⊥, N | ⊥)`, `composite`, composition label, `(agreeing, k)`, and the two sorts' own coordinates `ledger_composite | ⊥`, `bounded_composite | ⊥` with `floor_basis` and `floor_witness`.
- `power_min = min_j floor_basis_j` — the gated, sort-honest figure, and what `check_dependencies` compares against a caller's floor.
...
| V5 | Seal with `min_j floor_basis_j < p_min` — including a claim whose declared bounded figure clears the floor while its kernel-counted ledger figure does not |
...
**Established** = the escape's run has one identical replay (verdict and witness equal). A divergent replay **discredits** the checker in this ledger, monotonically, barring its future filings; and where the run it replayed had already established, it marks that run **contested**. **Valid** (read at query time, never stored): established ∧ not voided by a resolution ∧ (tier B ⇒ adjudicated accepted).

Neither the discredited set nor the Admission refusal registry appears in validity. A checker's discredit or refusal voids only what that checker never demonstrated, and an unestablished run is already invalid by the first conjunct, so the clause that once read those sets was subsumed and is gone (`_check_valid`; the removal is derived in `PROOFS.md`). What reaches an established escape is a divergence on *that* run and nothing else. Validity is therefore position-independent: it reads no lower-machine state, so no `as_of` bound is needed to keep a refusal recorded later from retroactively changing an earlier record.

**Contested** = a replay of an established run diverged from its filed outcome. The kernel cannot settle which report is true — both are harness reports (B1) about a component B2 assumes deterministic — so it refuses to let the later one win silently: the conflict is journaled, the escape keeps impeaching, and the outcome takes a **resolution**, a named and reasoned decision `uphold | void`, journaled once. This is the only route by which a line's standing rises.
...
| Name | Guard | Effect |
|---|---|---|
| FileEscape | `line ∈ S_R`; claim in the seal; checker declared, not discredited, and not refused in Admission **as of the cut this filing records**; `sha256(bytes) = seal.artifact_hash`; tier A: checker pinned to the claim in the seal and filed seed equals the kernel's derivation; verdict = `refuted` | escape recorded with tier, finder, position, and the scrutiny-journal cut it was filed at |
| ReplayEscape | escape exists; checker not discredited | equal `(verdict, witness)` → established; unequal → checker discredited, monotone, and if the run had established it is marked contested. Every **unestablished** escape of a discredited checker is void; an established one is not — voiding it takes a divergence on that run and a resolution |
| Adjudicate | tier-B escape established; named actor | decision `accept` or `reject`, with reason, journaled; `accept` makes it valid |
| Resolve | the run is contested and unresolved; named actor; reason | decision `uphold` or `void`, journaled once; `void` is the only transition that raises a line's standing |
...
| CalOpen | the class carries an explicit calibration policy (E9); no refuter pinned by the class is demoted here or refused in Admission | drives `Admission.open`, then journals the check as `cal_open`. Emitted only after the delegated transition succeeds, so a refused CalOpen leaves no trace; the event is what makes this gate auditable, and `mediated` requires it |
...
- **C1 Established, never asserted.** A tier-A escape has effect only past kernel-checked preconditions — seal membership, byte-hash equality, pinned checker, kernel-derived seed, refuted verdict, identical replay — none of which is a declaration; a tier-B escape has effect only past a journaled, attributed adjudication. No other path to effect exists. The same discipline governs the falsifier: what removes an established escape's effect is a divergence on that run plus an attributed resolution, never a report about another run.
...
- **C3 Impeachment is entailed; un-revocation is attributed.** `impeached(id)` is a pure query entailed by a valid escape, and the seal is never rewritten. The two directions are deliberately asymmetric, and this is the theorem's content. *Downward*: impeachment follows from a demonstration alone — a counterfactual re-run of the seal's own pinned instrument, replayed — and no authority can decline it. *Upward*: an established escape's effect is removed only by a divergence on that very run, which merely marks it contested and leaves it impeaching, plus a `cal_resolve` naming an actor and a reason. No single report, about any other run or from any other layer, raises a line's standing. Non-discretion is the property CRL/OCSP lack downward; what this adds is that the upward direction is not free either, because a kernel cannot adjudicate between two conflicting reports about a component it only assumes deterministic (B1, B2) and must therefore refuse to let the later one win in silence.
...
- **C5 Track record carried, and mediation provable at both gates.** Every CalSeal stamps the seal in the same step, and every CalOpen journals its own check; `mediated(id)` requires both — exactly one `cal_open` for the line, and exactly one stamp bound to the seal's own position. A line that took either transition directly on the lower machine is layer IR, `admissible` refuses it, and a consumer can tell the two apart. Reading the stamp alone was not enough: a line opened around this authority, and therefore never subjected to C6's open-time demotion gate, could be sealed through it and answer mediated on the strength of the stamp. Naive deletion of a `cal_open` fails the stamp's control total (each stamp `as_of` counts journal length). A deleter who prunes and refits the surviving stamps replays clean, and absence then fails closed to IR — a control total, not an anchor. Rebuild refuses an open that appears after that line's stamp, binds the open's `as_of` to the line's `opened_at`, and refuses an open that precedes a filing at that cut. A C6-green insert before the stamp, or a filing whose `as_of` was slid past the open, still replays once stamps are refitted — the same residue as a forged stamp in seal order. Re-pointing a stamp, duplicating one, or adding one out of seal order is refused on rebuild; adding one for an unmediated seal at that seal's own position in stamp order is not — later `as_of` primaries can be renumbered — and removing one lowers the line to IR. Both residues are stated in PROOFS.md rather than claimed away.
...
| Id | Forbidden step |
|---|---|
| E1 | Effect from an escape that is not established, or whose contest was resolved `void`; a filing by a checker discredited, or refused in Admission as of the filing's own recorded cut |
