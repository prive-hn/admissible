...
An accident worth recording turned up here. Deleting an open event is not merely standing-lowering but \emph{refused}, because every stamp carries the calibration journal's own length at stamping among its primaries, so dropping any earlier event makes that stamp fail to recompute --- a control total in the audit sense~\cite{crosby}. It holds only as far as a later stamp reaches: past the last stamp the total runs out, and deleting the final stamp is undetectable. We rely on the guard and not on the accident, and we state the boundary rather than the happy case.
