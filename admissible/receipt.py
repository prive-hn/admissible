...
from . import fsutil
from .decision import (ADMITTED, CHECKS_PASSED, Decision,
...
def load_signer(environment: dict[str, str] | None = None):
    ...
    raise SigningError(
        ...
        "ADMISSIBLE_HMAC_KEY_FILE at a file that only you can read. Use "
        "'--preview' to evaluate without issuing a receipt.")
...
def anchor(store, journal_id: str, event: dict, *, signer, now: int,
           attempts: int = 8,
           attach: Callable[[Proposal], Sequence[tuple[str, tuple]]] | None = None,
    ...
    before the attachment rows, so callers such as defect idempotency may
    ...
    for _ in range(attempts):
        ...
        attachments = tuple(attach(proposal)) if attach is not None else ()

        def builder(rows=attachments):
        ...
        except store_module.HeadConflict as error:
            last = error
            continue
        return proposal
    raise AnchorError(
...
def issue_receipt_from_parts(store, *, repository: str, commit_sha: str,
    ...
    if signer is None:
        raise SigningError(
            "issuing a receipt needs a signing key; set ADMISSIBLE_HMAC_KEY or "
            "use --preview")
    if state != ADMITTED:
        raise ReceiptError(
            f"a workflow receipt only ever records an admission; refusing to "
        ...
        for digest, receipts in binders.items():
            ...
            try:
                ...
                raise ReceiptError(
                    ...
                    f"{error}. Nothing was anchored.") from None
            actual = (row["kind"], row["repository"], row["commit_sha"],
                      row["tree_sha"], row["policy_digest"],
                      row["record_json"])
    ...
    def ensure_attachment_correspondence(*, require_present: bool) -> None:
        ...
        for digest, expected in evidence_rows.items():
            ...
            if stored is None:
                ...
                store.connection.execute(
                    "INSERT INTO evidence(digest, kind, repository, "
                    "commit_sha, tree_sha, policy_digest, record_json) "
                    "VALUES(?,?,?,?,?,?,?)",
                    (digest, expected["kind"], expected["repository"],
                     expected["commit_sha"], expected["tree_sha"],
                     expected["policy_digest"], expected["record_json"]))
                stored = store.connection.execute(
                    "SELECT kind, repository, commit_sha, tree_sha, "
                    "policy_digest, record_json FROM evidence WHERE digest=?",
                    (digest,)).fetchone()
            actual = None if stored is None else tuple(stored[key] for key in (
                "kind", "repository", "commit_sha", "tree_sha",
        ...
        for dependency_repository, dependency_sha in normalized_dependencies:
            ...
            parameters = (repository, commit_sha,
            ...
            if stored is None:
                ...
                stored = store.connection.execute(
                  ...
                  and stored["recorded_at"] == prior_time
                ...
                store.connection.execute(
                    ...
                    "dependency_repository=? AND dependency_commit_sha=? "
                    "AND recorded_at=?",
                    (expected_time,) + parameters + (prior_time,))
                stored = store.connection.execute(
                    "SELECT recorded_at FROM dependencies WHERE "
                    "consumer_repository=? AND consumer_commit_sha=? AND "
                    "dependency_repository=? AND dependency_commit_sha=?",
                    parameters).fetchone()
            if stored is None or stored["recorded_at"] != expected_time:
                raise ReceiptError(
        ...
        rows = store.connection.execute(
            "SELECT dependency_repository, dependency_commit_sha, "
            "recorded_at FROM dependencies WHERE consumer_repository=? AND "
            "consumer_commit_sha=?",
    ...
    def authenticated_repository_preflight() -> None:
        ...
        try:
            current = store.current_head(journal_id)
            if current is None:
                orphaned_authority = any(
                    store.connection.execute(query, parameters).fetchone()
                    is not None for query, parameters in (
                    ("SELECT 1 FROM journal_events WHERE journal_id=? LIMIT 1",
                     (journal_id,)),
                    ("SELECT 1 FROM head_receipts WHERE journal_id=? LIMIT 1",
                     (journal_id,)),
                    ("SELECT 1 FROM workflow_receipts WHERE repository=? "
                     "LIMIT 1", (repository,)),
                    ("SELECT 1 FROM defects WHERE repository=? LIMIT 1",
                     (repository,)),
                ))
                if orphaned_authority:
                    raise ReceiptError(
                ...
                dependency_rows = store.connection.execute(
                    "SELECT consumer_repository, consumer_commit_sha, "
                    "dependency_repository, dependency_commit_sha, "
                    "recorded_at FROM dependencies WHERE "
                    "consumer_repository=?", (repository,)).fetchall()
                for row in dependency_rows:
                    edge = (row["consumer_repository"],
                            row["consumer_commit_sha"],
                            row["dependency_repository"],
                            row["dependency_commit_sha"])
                ...
                return
            store._authenticated_repository_projection_locked(
                repository, signer)
        except ReceiptError:
            raise
    ...
    def receipt_transaction_preflight() -> None:
        # Both checks must precede Store.accept_head's exact-head idempotency
        # return. Two writers can precompute the same proposal; after the
        # winner commits, repository attachments or policy authority can
        # change before the loser acquires the lock. Returning merely because
        # the signed head now matches would skip cached receipt authentication
        # and could report ADMITTED from a namespace standing calls UNKNOWN.
    ...
    def attach(proposal: Proposal):
        ...
        rows = [store.workflow_receipt_row(receipt)]
        return rows
