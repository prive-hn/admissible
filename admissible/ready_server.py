...
The service is loopback-only by CLI policy.  It presents the same Ready document
used by the CLI and MCP tools; it never receives signing credentials or issues an
admission.
"""
...
class ReadyService:
    ...
    def state(self) -> dict[str, Any]:
        return ready_module.inspect(self.repo)
...
def _handler(service: ReadyService):
    class Handler(BaseHTTPRequestHandler):
        server_version = "AdmissibleReady/0.7"
        ...
        def do_GET(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API
            ...
            try:
                body = resources.files("admissible.ready_static").joinpath(
                    name).read_bytes()
