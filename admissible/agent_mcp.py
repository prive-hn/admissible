...
The server exposes a bounded workflow contract, never trusted admission authority.
All diagnostics belong on stderr; :func:`serve_stdio` writes JSON-RPC only.
"""
...
class Server:
    ...
    def _call_tool(self, name: str,
        ...
        if name == "admissible_check":
            ...
            try:
                current = identity_module.repository_identity(self.repo)
            ...
            _, document = ready_module.run_check(
                ...
                package=identity)
            return document
        state = ready_module.inspect(self.repo)
        return {
