...
def _ci_body(provider: str, tool_sha: object, allow_placeholder: bool) -> str:
    ...
    template = (Path(__file__).resolve().parent / "templates"
                / CI_PROVIDERS[provider][1])
    try:
...
def plan_init(root: Path | str, profile_name: str, *, ci: str | None = None,
              ...
              gitignore: bool = True) -> tuple[InitWrite, ...]:
    ...
    if ci is not None:
        ...
        body = _ci_body(ci, tool_sha, allow_placeholder)
