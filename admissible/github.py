"""GitHub Actions boundaries: evaluate is untrusted, finalize is trusted.

Two rules make this safe enough to describe honestly:
...
1. **Bind to the head commit, never to the synthetic merge commit.** On
   ...
   verified again, so this module reads
   ``event.pull_request.head.sha`` and refuses anything that is not a full
...
from .decision import (ADMITTED, CHECKS_PASSED, DECISION_SCOPE,
                       MAX_CLOCK_SKEW_SECONDS, READINESS,
                       READINESS_AWAITING_REVIEW,
                       READINESS_READY_FOR_ATTESTATION, digest_of_document,
                       evaluate)
...
def require_trusted_policy(store, *, repository: str, class_id: str,
    ...
    from .config import enforcement_digest
...
def preview_document(*, repository: str, commit_sha: str, tree_sha: str,
    ...
    try:
        attestation_module.evaluation_state_readiness(state, readiness)
...
def assert_trusted_tool(policy_root: Path | str | None) -> bool:
    ...
    The finalizer holds the signing key. Once the ``admissible`` package Python
    ...
    * the candidate checkout ships a top-level ``admissible`` package of its
      own, which is one ``cd`` away from being the program that signs.
    """
    ...
    if package == root or root in package.parents:
        raise GitHubError(
            ...
            "finalize from there.")
    for shadow in (root / "admissible" / "__init__.py", root / "admissible.py"):
        if shadow.exists():
            raise GitHubError(
                f"refusing to finalize: the checkout under evaluation ({root}) "
                f"ships its own {shadow.name} at {shadow.parent}. Running the "
                "finalizer from that directory would hand the signing key to "
                "the candidate's code before any check of ours could run. Keep "
                "the tool checkout and the candidate checkout separate, and "
                "point --policy-root at a checkout that carries policy and "
                "data only.")
    return True
...
def _dependency_edges(value: object) -> list[dict[str, str]]:
    ...
    for item in value:
        ...
        edges.append({"repository": item["repository"],
...
def _validated_finalization(
    ...
    try:
        observed = identity_module.repository_identity(
    ...
    from .decision import decision_to_dict
    ...
    from .runner import ISOLATION_MODES, ISOLATION_NONE
