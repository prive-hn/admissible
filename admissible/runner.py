...
from .fsutil import PathError, resolve_within
...
# the observer signs that field with everything else, and ``finalize`` refuses a
...
def declared_isolation(source: dict[str, str] | None = None) -> str:
    ...
    process group and is refused by ``finalize``. An unrecognised value is a
...
def ambient_signing_credentials(source: dict[str, str] | None = None
    ...
    """Every signing credential visible to this process, in declared order."""
